...
class OperationReport extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      totTickets: null, totSaleData: [], revTM: [], revLY: [],
    }
  }
  ...
  async fetchData(values) {
    ...
    if (values.queryType === 'totTickets') {
      response = await fetch('http://localhost:3000/api/staff/report', requestOptions).then((r) => r.json())
      responseMonth = await fetch('http://localhost:3000/api/staff/reportByMonth', requestOptions).then((r) => r.json())
      ...
      responseDirect = await fetch('http://localhost:3000/api/staff/revenueDirect', requestOptions).then((r) => r.json())
      responseIndirect = await fetch('http://localhost:3000/api/staff/revenueIndirect', requestOptions).then((r) => r.json())
  ...
  render() {
    const onFinish = async (values) => {
      ...
      wrappedVals.queryType = 'totTickets'
      this.fetchData(wrappedVals)
    }
    return (
      <div>
        <Row
...
class StaffMyFlightTable extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      searched: false, data: [], queryType: this.props.queryType, columns: staffFlightColumns,
    }
  }
...
class StaffPage extends React.Component {
  ...
  async checkLoginStatus() {
    ...
    const role = await Cookies.get('adminRole') !== 'Airline Staff'
    if (loggedIn || role) {
      ...
      console.log()
      const loginCheckResponse = await fetch('http://localhost:3000/api/staff/auth', checkOptions)
  ...
  render() {
    ...
    if (this.state.currentPanel === '0') {
      ...
      contentWrapper = (
        <div style={{ width: '100%' }}>
          <StaffMyFlightTable queryType="staff_My_Flight" onLoadQuery />
        </div>
      )
