...
import {
  UserOutlined, WalletOutlined, GlobalOutlined, HomeOutlined,
...
const breadcrumbMatch = {
  0: 'Home', 1: 'Flights', 2: 'Flights', 3: 'Customer', 4: 'Commission',
}
...
class Commission extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      commLT: 0, avgLT: 0, commCus: 0, avgCus: 0,
    }
  }
  ...
  async fetchData(values) {
    ...
    if (values.queryType === 'commDefault') {
      ...
      this.setState({ commCus, avgCus: commCus / response.length })
      ReactDOM.render(
        (
          <div>
...
class TopCustomers extends React.Component {
  ...
  render() {
    const onFinish = async (values) => {
      ...
      const responsePur = await fetch('http://localhost:3000/api/agent/fathers', requestOptions).then((response) => response.json()).catch(() => {
        alert('Query Failed!')
      })
      const responseCom = await fetch('http://localhost:3000/api/agent/mothers', requestOptions).then((response) => response.json()).catch(() => {
        alert('Query Failed!')
      })
...
function PurchaseModal(props) {
  ...
  const onFinish = async () => {
    ...
    const response = await fetch('http://localhost:3000/api/agent/purchase', requestOptions)
...
class AgentMyFlightTable extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      searched: false, data: [], queryType: this.props.queryType, columns: agentFlightColumns,
    }
  }
...
class QueryFlightTable extends React.Component {
  ...
  async fetchData(values) {
    ...
    response = await fetch('http://localhost:3000/api/search/flight', requestOptions).then((response) => response.json()).catch(() => { this.setState({ data: [] }) })
    for (let i = 0; i < response.length; i++) {
      response[i].departure_time = response[i].departure_time.slice(0, -5)
      response[i].arrival_time = response[i].arrival_time.slice(0, -5)
    }
...
class AgentPage extends React.Component {
  ...
  async checkLoginStatus() {
    const loggedIn = await Cookies.get('adminLoggedIn') !== 'true'
    const role = await Cookies.get('adminRole') !== 'Booking Agent'
    if (loggedIn || role) {
      ...
      console.log()
      const loginCheckResponse = await fetch('http://localhost:3000/api/agent/flight', checkOptions)

      if (loginCheckResponse.status !== 200) {
        await alert('User authentication failed, please login again!')
        this.props.history.push('/admin')
      } else {
        const loginChkResponse_json = loginCheckResponse.json()
  ...
  render() {
    ...
    if (this.state.currentPanel === '0') {
      ...
      contentWrapper = (
        <div style={{ width: '100%' }}>
          <AgentMyFlightTable queryType="agent_My_Flight" onLoadQuery />
        </div>
      )
